...
public class InstitucionConverter implements Converter {
    ...
    private static final Logger LOGGER = Logger.getLogger(InstitucionConverter.class.getName());
    //</editor-fold>
    ...
    public Object getAsObject(FacesContext fc, UIComponent uic, String value) {
        if (!value.equals("null") && value.trim().length() > 0) {
            try {
                return new AdmInstitucion(Integer.parseInt(value));
            } catch (NumberFormatException ex) {
            } catch (Exception ex) {
                Logger.getLogger(InstitucionConverter.class.getName()).log(Level.SEVERE, null, ex);
            }
            return null;
        } else {
            return null;
    ...
    public String getAsString(FacesContext fc, UIComponent uic, Object object) {
        if (object != null && object != "") {
            return String.valueOf(((AdmInstitucion) object).getIdInstitucion());
