 ...
 * @author jcerda
 */
@FacesConverter("variableConverter")
public class VariableConverter implements Converter{
    ...
    public Object getAsObject(FacesContext fc, UIComponent uic, String value) {
        if (value != null && value.trim().length() > 0) {
            try {
                return new MetVariable(Integer.parseInt(value));
            } catch (NumberFormatException es) {
            } catch (Exception ex) {
                Logger.getLogger(VariableConverter.class.getName()).log(Level.SEVERE, null, ex);
            }
            return null;
        } else {
            return null;
    ...
    public String getAsString(FacesContext fc, UIComponent uic, Object object) {
        if (object != null && object != "") {
            return String.valueOf(((MetVariable) object).getIdVar());
