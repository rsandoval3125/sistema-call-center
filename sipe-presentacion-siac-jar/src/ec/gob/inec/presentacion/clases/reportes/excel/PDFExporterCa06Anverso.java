 ...
 * @since 0.7.0
 */
public class PDFExporterCa06Anverso extends Exporter {
        ...
        try {
            ...
            while (st.hasMoreElements()) {
                ...
                if (!lstFor2.isEmpty()) {
                    ...
                    table.setWidths(new float[]{7, 13});
                    ...
                    document.add(tablaForFiltros(indexLstFor2NoIncremental, indexLstFor2, lstFor2, resumenFiltro, resumenFiltroFor2, boldFor2, normalFor2, cell, table));
                }
                ...
                PdfPTable tableVertice = new PdfPTable(1);
                tableVertice.setTotalWidth(425);
                tableVertice.setLockedWidth(true);
                tableVertice.setWidths(new float[]{425});
                tableVertice.setHorizontalAlignment(Element.ALIGN_LEFT);

                PdfPCell cell = new PdfPCell();
                cell = new PdfPCell(new Paragraph("DESCRIPCIÓN DE LÍMITES DEL SECTOR", boldFor2));
                cell.setHorizontalAlignment(Element.ALIGN_CENTER);
                cell.setVerticalAlignment(Element.ALIGN_MIDDLE);
                tableVertice.addCell(cell);

                for (int m = 1; m < lstFor2.size(); m++) {
                    if (lstFor2.get(m).length == 1) {
                        cell = new PdfPCell(new Paragraph(String.valueOf(lstFor2.get(m)[0]), normalFor2));
                        cell.setHorizontalAlignment(Element.ALIGN_LEFT);
                        cell.setVerticalAlignment(Element.ALIGN_MIDDLE);
                        tableVertice.addCell(cell);
                    }
                }
                cell = new PdfPCell(new Paragraph("SIMBOLOGÍA", boldFor2));
                cell.setHorizontalAlignment(Element.ALIGN_CENTER);
                cell.setVerticalAlignment(Element.ALIGN_MIDDLE);
                tableVertice.addCell(cell);

                cell = new PdfPCell(imagen(document, "/welcome-content/imagenes/simbologiaCa06AV_V2.png"), false);
                tableVertice.addCell(cell);

                // añado a la primera celda de la tabla principal la tabla 1
                firstTableCell.addElement(tableVertice);
                ...
                pdf.setTotalWidth(700);//2300-A1**               
                pdf.setLockedWidth(true);
                pdf.setHorizontalAlignment(Element.ALIGN_RIGHT);
                ...
                /*PdfPTable tableResponsables = new PdfPTable(3);
                tableResponsables.setTotalWidth(575);
                tableResponsables.setLockedWidth(true);
                tableResponsables.setHorizontalAlignment(Element.ALIGN_RIGHT);
                tableResponsables.setWidths(new float[]{13, 13, 13});*/
                PdfPCell cellResponsables = new PdfPCell();

                cellResponsables = new PdfPCell(new Paragraph("RESPONSABLES", boldFor2));
                cellResponsables.setHorizontalAlignment(Element.ALIGN_CENTER);
                cellResponsables.setVerticalAlignment(Element.ALIGN_MIDDLE);
                cellResponsables.setColspan(3);
                pdf.addCell(cellResponsables);

                cellResponsables = new PdfPCell(new Paragraph("NOMBRES", boldFor2));
                cellResponsables.setHorizontalAlignment(Element.ALIGN_CENTER);
                cellResponsables.setVerticalAlignment(Element.ALIGN_MIDDLE);
                cellResponsables.setColspan(6);
                pdf.addCell(cellResponsables);

                cellResponsables = new PdfPCell(new Paragraph("DOMICILIO", boldFor2));
                cellResponsables.setHorizontalAlignment(Element.ALIGN_CENTER);
                cellResponsables.setVerticalAlignment(Element.ALIGN_MIDDLE);
                cellResponsables.setColspan(1);
                pdf.addCell(cellResponsables);

                //document.add(tablaForDetalleResponsables(indexLstFor2NoIncremental, indexLstFor2, lstFor2, resumenFiltro, resumenFiltroFor2, boldFor2, normalFor2, cell, pdf));
                // añado a la segunda celda de la tabla principal la tabla 2
                secondTableCell.addElement(tablaForDetalleResponsables(indexLstFor2NoIncremental, indexLstFor2, lstFor2, resumenFiltro, resumenFiltroFor2, boldFor2, normalFor2, cell, pdf));
    ...
    public PdfPTable tablaForFiltros(int indexLstFor2NoIncremental, int indexLstFor2, List<Object[]> lstFor2, List<Object[]> resumenFiltro, List<Object[]> resumenFiltroFor2, Font boldFor2, Font normalFor2, PdfPCell cell, PdfPTable table) {
        resumenFiltro.remove(6);
    ...
    public PdfPTable tablaForDetalleResponsables(int indexLstFor2NoIncremental, int indexLstFor2, List<Object[]> lstFor2, List<Object[]> resumenFiltro, List<Object[]> resumenFiltroFor2, Font boldFor2, Font normalFor2, PdfPCell cell, PdfPTable table) {
        int i = 0;
        for (int m = 0; m < lstFor2.get(0).length; m++) {
            //if (indexLstFor2 < lstFor2.size()) {
            if (i < 5) {
                if (i == 0) {
                    cell = new PdfPCell(new Paragraph("JEFE DEL SECTOR", boldFor2));
                } else if (i == 2) {
                    cell = new PdfPCell(new Paragraph("AYUDANTE", boldFor2));
                } else if (i == 4) {
                    cell = new PdfPCell(new Paragraph("AYUDANTE", boldFor2));
                }
                cell.setHorizontalAlignment(Element.ALIGN_LEFT);
                ...
                String nom;
                if (String.valueOf(lstFor2.get(0)[i]).equals("null")) {
                    nom = "";
                } else {
                    nom = String.valueOf(lstFor2.get(0)[i]);
                }
                cell = new PdfPCell(new Paragraph(nom, normalFor2));
                cell.setHorizontalAlignment(Element.ALIGN_CENTER);
                cell.setVerticalAlignment(Element.ALIGN_MIDDLE);
                cell.setColspan(6);
                table.addCell(cell);
                ...
                String dom;
                if (String.valueOf(lstFor2.get(0)[i + 1]).equals("null")) {
                    dom = "";
                } else {
                    dom = String.valueOf(lstFor2.get(0)[i + 1]);
                }
                cell = new PdfPCell(new Paragraph(dom, normalFor2));
                cell.setHorizontalAlignment(Element.ALIGN_CENTER);
                cell.setVerticalAlignment(Element.ALIGN_MIDDLE);
                cell.setColspan(1);
                table.addCell(cell);
            ...
            if (i == 6) {
                ...
                if (String.valueOf(lstFor2.get(0)[i]).equals("null")) {
                    ...
                    act = "ACTUALIZADOR: " + String.valueOf(lstFor2.get(0)[i]);
                }
                cell = new PdfPCell(new Paragraph(act, normalFor2));
                cell.setHorizontalAlignment(Element.ALIGN_LEFT);
                cell.setVerticalAlignment(Element.ALIGN_MIDDLE);
                cell.setColspan(3);
                table.addCell(cell);

                if (String.valueOf(lstFor2.get(0)[i + 2]).equals("null")) {
                    sup = "SUPERVISOR: ";
                } else {
                    sup = "SUPERVISOR: " + String.valueOf(lstFor2.get(0)[i + 2]);
                }
                cell = new PdfPCell(new Paragraph(sup, normalFor2));
                cell.setHorizontalAlignment(Element.ALIGN_LEFT);
                cell.setVerticalAlignment(Element.ALIGN_MIDDLE);
                cell.setColspan(6);
                table.addCell(cell);

                if (String.valueOf(lstFor2.get(0)[i + 4]).equals("null")) {
                    dig = "DIGITALIZADOR: ";
                } else {
                    dig = "DIGITALIZADOR: " + String.valueOf(lstFor2.get(0)[i + 4]);
                }
                cell = new PdfPCell(new Paragraph(dig, normalFor2));
                cell.setHorizontalAlignment(Element.ALIGN_LEFT);
                cell.setVerticalAlignment(Element.ALIGN_MIDDLE);
                cell.setColspan(1);
                table.addCell(cell);
            ...
            if (i == 8) {
                ...
                if (String.valueOf(lstFor2.get(0)[i - 1]).equals("null")) {
                    ...
                    act = "NOMBRE: " + String.valueOf(lstFor2.get(0)[i - 1]);
                }
                cell = new PdfPCell(new Paragraph(act, normalFor2));
                cell.setHorizontalAlignment(Element.ALIGN_LEFT);
                cell.setVerticalAlignment(Element.ALIGN_MIDDLE);
                ...
                if (String.valueOf(lstFor2.get(0)[i + 1]).equals("null")) {
                    ...
                    sup = "NOMBRE: " + String.valueOf(lstFor2.get(0)[i + 1]);
                }
                cell = new PdfPCell(new Paragraph(sup, normalFor2));
                cell.setHorizontalAlignment(Element.ALIGN_LEFT);
                cell.setVerticalAlignment(Element.ALIGN_MIDDLE);
                ...
                if (String.valueOf(lstFor2.get(0)[i + 3]).equals("null")) {
                    dig = "NOMBRE: ";
                } else {
                    dig = "NOMBRE: " + String.valueOf(lstFor2.get(0)[i + 3]);
                }
                cell = new PdfPCell(new Paragraph(dig, normalFor2));
                cell.setHorizontalAlignment(Element.ALIGN_LEFT);
                cell.setVerticalAlignment(Element.ALIGN_MIDDLE);
                ...
                cell = new PdfPCell(new Paragraph("FIRMA: ________________________", normalFor2));
                cell.setHorizontalAlignment(Element.ALIGN_LEFT);
                cell.setVerticalAlignment(Element.ALIGN_MIDDLE);
                cell.setColspan(3);
                table.addCell(cell);
            ...
            if (i == 10) {
                ...
                if (String.valueOf(lstFor2.get(0)[i + 2]).equals("null")) {
                    ...
                    act = "¿CUAL ES LA MEJOR VÍA PARA LLEGAR AL SECTOR? " + String.valueOf(lstFor2.get(0)[i + 2]);
                }
                cell = new PdfPCell(new Paragraph(act, normalFor2));
                cell.setHorizontalAlignment(Element.ALIGN_LEFT);
                cell.setVerticalAlignment(Element.ALIGN_MIDDLE);
                ...
                cell = new PdfPCell(new Paragraph("NOTA: TOME LA INFORMACIÓN DEL FORMULARIO Ca-06 (ANEXO-A)", boldFor2));
                cell.setHorizontalAlignment(Element.ALIGN_LEFT);
                cell.setVerticalAlignment(Element.ALIGN_MIDDLE);
                cell.setColspan(10);
                table.addCell(cell);
                ...
                if (component instanceof Row) {
                    ...
                    for (UIComponent rowComponent : row.getChildren()) {
                        ...
                        if (column.isRendered() && column.isExportable()) {
                            ...
                            if (facetType.equalsIgnoreCase("footer")) {
                                pdfTable.addCell(cell1);
                            }
                        }
                        col++;
    ...
    public Image imagen(Document document, String dir) {
        try {
            String fileName = System.getProperty("jboss.home.dir") + dir;
            Image imagen = Image.getInstance(fileName);
            imagen.scaleAbsoluteHeight(200f);
            imagen.scaleAbsoluteWidth(425);
            imagen.setBorder(Rectangle.BOX);
            imagen.setBorderWidth(1f);
            /*imagen.scaleAbsoluteHeight(60f);//80
            imagen.scaleAbsoluteWidth(190);//250
            imagen.setAbsolutePosition(20, 770);//40-1500*/
            //imagen.setAlignment(Element.ALIGN_LEFT);
